...
full, adding a new object to the queue requires getting rid of an existing object; 
with a circular queue, the object that has been in the queue the longest is 
discarded and replaced by the new object.

Assuming we have a circular queue with room for 3 objects, the circular queue looks
and acts like this:
...
to the constructor, and should provide the following methods:

enqueue to add an object to the queue
dequeue to remove (and return) the oldest object in the queue. It should return 
null if the queue is empty.

You may assume that none of the values stored in the queue are null (however, null 
...
CircularQueue.prototype.enqueue = function(object) {
  if (this.queue[this.latestSpot] !== null) {
    this.oldestSpot = this.nextSpot(this.latestSpot);
  }
  this.queue[this.latestSpot] = object;
  this.latestSpot = this.nextSpot(this.latestSpot);
}

CircularQueue.prototype.dequeue = function() {
  let oldestSpotValue = this.queue[this.oldestSpot];
  this.queue[this.oldestSpot] = null;
  if (oldestSpotValue !== null) {
    this.oldestSpot = this.nextSpot(this.oldestSpot);
  }
  return oldestSpotValue;
}

CircularQueue.prototype.nextSpot = function(spot) {
  if ((spot + 1) >= this.queue.length) return spot = 0;
  else return spot += 1;
}
*/

function CircularQueue(queueLength) {
  this.queue = [];
...
CircularQueue.prototype.enqueue = function(object) {
  ...
  if (this.queue.length > this.queueLength) this.queue.shift();
}
