...
Output: number representing the duplicate
...
for each number in the array
  we check if it occurs in the rest of the array index + 1 to array length
    if we have a match, we return the value



function findDup(array) {
  for (let index = 0; index < array.length; index += 1) {
    for (let indexRest = index + 1; indexRest < array.length; indexRest += 1) {
      if (array[index] === array[indexRest]) {
        return array[index];
      }
    }
  }
